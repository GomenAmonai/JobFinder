...
Docker Compose. (Phase 3: JWT-авторизация + сохранённые фильтры. Phase 4:
отклики. Phase 5: парсинг зарплат, кросс-источниковый дедуп, коллектор RemoteOK.
Посредник: роли + нативные вакансии работодателей.)
...
Отклики (требуют авторизации, скоупятся по пользователю):

- `POST /vacancies/{id}/applications` — откликнуться (повторно → 409)
- `GET /me/applications` — мой пайплайн откликов
- `GET /me/applications/{id}`
- `PATCH /me/applications/{id}/status` — смена статуса (валидируется переход; устаревшая версия → 409)
- `DELETE /me/applications/{id}` — убрать из пайплайна

Работодатель (требуют роли Employer):

- `POST /employer/vacancies` — запостить нативную вакансию на JobRadar
- `GET /employer/applications` — отклики на свои вакансии
- `PATCH /employer/applications/{id}/status` — двигать статус отклика (кандидат получает push)

Дедуп и зарплата: вакансии с разных источников сворачиваются по нормализованному
`company|title` (видна каноническая, первая увиденная); нативные вакансии в дедуп
не входят. `SalaryRaw` парсится в `SalaryMin/Max/Currency` на приёме.
